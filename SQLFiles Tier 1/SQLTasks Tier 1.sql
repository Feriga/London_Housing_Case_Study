...
SELECT name, monthlymaintenance as cost,
	CASE 
...
SELECT CONCAT_WS(" ", firstname, surname), MAX(joindate) as last_signed_up
FROM Members
WHERE firstname != 'GUEST' or surname != 'GUEST'
...
SELECT s1.court, CONCAT_WS(" ", s1.firstname, s1.surname) AS name 
FROM 
...
INNER JOIN Members ON Bookings.memid = Members.memid) s1
GROUP BY s1.court, s1.firstname, s1.surname
ORDER BY name
...
FROM (
SELECT CAST(SUBSTR(starttime, 6, 2)  AS months, memid
FROM Bookings
WHERE memid !=0
) AS b
GROUP BY b.months;
